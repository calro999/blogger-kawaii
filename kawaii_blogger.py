...
def generate_room_comment_with_llm(item):
    ...
    prompt = f"""以下のファンシー・キャラクターカワイイ商品情報を基にして、楽天ROOM用の魅力的な紹介コメント（400文字以内）を生成してください。
【商品名】: {title}
【価格】: {price}円
【商品説明・特徴】: {caption[:200]}

以下の要件を厳格に遵守してください：
1. 口調・トーン：カワイイ雑貨・キャラクターファンに刺さる自然で愛のある語り口とし、「これかわいい！」「これ気になってた！」「これ便利だよ！」といった安易な定型表現は絶対に使用しないでください。
2. 文字数：400文字以内（厳守。超えると投稿エラーになります）。
3. 絵文字：5〜8個使用して華やかにすること。
4. ハッシュタグ：3〜5個（「#カワイイ #キャラグッズ #推し活」等）含め、末尾に「#楽天市場」を必ず含めること。
5. URLや疑似リンク、プレースホルダー（「[リンクはこちら]」など）は絶対に含めないでください。
6. 出力は紹介コメントのテキストのみとし、前置きやMarkdownの装飾コードブロック等は一切含めないでください。
"""

    system_message = (
        "あなたはカワイイキャラクター・ファンシー雑貨専門のトップレビューブロガー・インフルエンサーです。"
        "定型フレーズを排し、ときめくデザインや実用性の高さを伝えるオリジナル紹介文を作成してください。"
    )
    ...
    starters = [
        f"持っているだけでキュンとする『{clean_title}』をピックアップ💕",
        f"カワイイ好き必見！『{clean_title}』の愛らしいデザインがたまりません✨",
        f"毎日が楽しくなる注目アイテム『{clean_title}』をご紹介！自分用やプレゼントにも最高👍",
        f"ファン垂涎の胸キュン商材『{clean_title}』を見つけて即チェック🎁",
        f"クオリティが高くておすすめの『{clean_title}』！気になる方はお早めにチェック✨"
    ]
    starter = random.choice(starters)

    bodies = [
        "見るたびに癒やされるキュートなデザイン！お部屋や持ち物に追加したくなります😊",
        "細部まで作り込まれていて愛らしさ抜群！持っているだけで気分が上がるアイテム✨",
        "評価が高くプレゼントしても確実に喜ばれる大満足のキャラクターグッズ🛍️",
        "お気に入りのコレクションに加えてぜひ楽しんでみてください👍"
    ]
